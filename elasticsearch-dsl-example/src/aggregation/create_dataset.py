...
def gen_data(times=1000 * 1000, player=200 * 1000, index="tennis"):
    ...
    for i in range(player):
        yield {"_index": index, "winner": f"player-{i}"}

    # such total to times
    for _ in range(times - player):
        yield {"_index": index, "winner": f"player-{random.randint(0, player)}"}
...
class TennisGameTemplate(Document):
    ...
    class Index:
        ...
        # src: https://github.com/elastic/elasticsearch-dsl-py/blob/
        #      58c02732658018333cfd5fa2f7b70d942773b0be/elasticsearch_dsl/index.py#L27
...
def DocFactory(index=None, index_settings=None):
    ...
    index_cls = new_cls._index
    index_cls._name = index_name
